...
namespace GeoSkylines
{
    public class GeoSkylinesImport
    {
        ...
        private string impTreesXmlFileName = "trees.xml";
        ...
        private string[] impTreesXmlRawTreeTypes;
        ...
        private int impTreesRasterTreesLimit;

        private ushort impTreesCoordMax = 8640;

        private Dictionary<int, TreeInfo[]> impTreesXmlTreeTypes;
        private ushort impTreesXmlOffsetX;
        private ushort impTreesXmlOffsetY;
        private int impTreesXmlMultiply;
        private int impTreesXmlTreesLimit;
        ...
        public void LoadConfiguration() {
            ...
            foreach (KeyValuePair<string, string> a_conf in conf) {
                ...
                if (key == "MapName")
                ...
                else if (key == "ImportTreesXmlTreeTypes") {
                    Dictionary<uint, string> prefab__number_name = new Dictionary<uint, string>();
                    ...
                    for (uint i = 0; i < prefabCnt; i++) {
                        ...
                        prefab__number_name[i] = prefab.name;
                    }

                    impTreesXmlRawTreeTypes = val.Split(',').Select(p => p.Trim()).ToArray();
                    Dictionary<int, List<TreeInfo>> d_treeTypes = new Dictionary<int, List<TreeInfo>>();

                    List<TreeInfo> l_treeTypes = new List<TreeInfo>();

                    foreach (var treeTypesPerID in impTreesXmlRawTreeTypes) {

                        int _id = 0;
                        string _s = treeTypesPerID;
                        int _index = _s.IndexOf('{');
                        int.TryParse(_s.Substring(1, _index - 1), out _id);
                        _s = _s.Substring(_index);
                        _s = _s.Substring(1, _s.Length - 3);
                        string[] _treeIDs = _s.Split('|');

                        if (!d_treeTypes.TryGetValue(_id, out l_treeTypes)) {
                            l_treeTypes = new List<TreeInfo>();
                            d_treeTypes.Add(_id, l_treeTypes);
                        }

                        foreach (var treeID in _treeIDs) {
                            if (prefab__number_name.ContainsKey(uint.Parse(treeID))){
                                l_treeTypes.Add(PrefabCollection<TreeInfo>.GetLoaded(uint.Parse(treeID)));
                            }
                        }
                        ...
                        d_treeTypes[_id] = l_treeTypes.ToList();

                    }

                    impTreesXmlTreeTypes = new Dictionary<int, TreeInfo[]>();
                    foreach(int id in d_treeTypes.Keys) {
                        impTreesXmlTreeTypes.Add(id, d_treeTypes[id].ToArray());
                    }

                } else if (key == "ImportTreesXmlOffsetX")
                    ushort.TryParse(val, out impTreesXmlOffsetX);
                else if (key == "ImportTreesXmlOffsetY")
                    ushort.TryParse(val, out impTreesXmlOffsetY);
                else if (key == "ImportTreesXmlMultiply")
                    int.TryParse(val, out impTreesXmlMultiply);
                else if (key == "ImportTreesXmlTreesLimit")
                    int.TryParse(val, out impTreesXmlTreesLimit);
                ...
                else if (key == "ImportTreesRasterTreesLimit")
                    int.TryParse(val, out impTreesRasterTreesLimit);
        ...
        public void ImportRoads() {
            ...
            string roadMappingFilePath = Path.GetFullPath("Files/" + roadMappingFileName.Trim());
            if (!File.Exists(roadMappingFilePath)) {
                panel.SetMessage("GeoSkylines", roadMappingFileName + " file doesn't exist!", false);
                return;
            } else {
                StreamReader r_map_sr = File.OpenText(roadMappingFilePath);
            ...
            string impRoadsFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impRoadsFileName.Trim()));
            if (!File.Exists(impRoadsFilePath)) {
                panel.SetMessage("GeoSkylines", impRoadsFileName + " file doesn't exist!", false);
                return;
            }

            StreamReader sr = File.OpenText(impRoadsFilePath);
        ...
        public void ImportRails() {
            ...
            string railMappingFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", railMappingFileName.Trim()));
            if (!File.Exists(railMappingFilePath)) {
                panel.SetMessage("GeoSkylines", railMappingFileName + " file doesn't exist!", false);
                return;
            } else {
                StreamReader r_map_sr = File.OpenText(railMappingFilePath);
                while (!r_map_sr.EndOfStream) {
                    var r_map_vec = CSVParser.Split(r_map_sr.ReadLine());
                    railMapping[r_map_vec[0]] = r_map_vec[1];
                }
            }
            string impRailsFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impRailsFileName.Trim()));
            if (!File.Exists(impRailsFilePath)) {
                panel.SetMessage("GeoSkylines", impRailsFileName + " file doesn't exist!", false);
                return;
            }

            StreamReader sr = File.OpenText(impRailsFilePath);
        ...
        public void ImportBuildings() {
            ...
            string impBuildingsFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impBuildingsFileName.Trim()));
            List<GeoSkylinesBuilding> buildings = new List<GeoSkylinesBuilding>();
            if (!File.Exists(impBuildingsFilePath)) {
                panel.SetMessage("GeoSkylines", impBuildingsFileName + " file doesn't exist!", false);
                return;
            }

            StreamReader sr = File.OpenText(impBuildingsFilePath);
        ...
        public void ImportTreesXML() {
            if (!confloaded)
                return;

            string xmlFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impTreesXmlFileName.Trim()));
            if (!File.Exists(xmlFilePath)) {
                panel.SetMessage("GeoSkylines", xmlFilePath + " file doesn't exist!", false);
                return;
            }

            TreesXML treesXML;
            Debug.Log("GeoSkylines: Reading tree.xml");
            using(FileStream file = new FileStream(xmlFilePath, FileMode.Open)) {
                XmlSerializer xmlSerializer = new XmlSerializer(typeof(TreesXML));
                treesXML = (TreesXML)xmlSerializer.Deserialize(file);
            }

            int trees_found = 0;

            int step = 999999999;
            if (impTreesXmlMultiply != 0) {
                step = impTreesXmlMultiply;
            }

            int treeLimit = 250000;
            if (impTreesXmlTreesLimit != 0) {
                treeLimit = impTreesXmlTreesLimit;
            }

            treeLimit -= TreeManager.instance.m_treeCount;

            int id = 0;

            var height = treesXML.Height;
            var width = treesXML.Width;
            Debug.Log("height: " + height.ToString() + " width: " + width.ToString());

            if (height != width) {
                string msg = "The size of the provided treeXML is not equal " + height + " =/= " + width;
            ...
            bool isRasterMode = treesXML.Mode == "raster";

            //List<Vector3> treePos;
            TreeInfo[] treeInfo;

            foreach(BufferedTreesCollection bufferedTreesCollection in treesXML.Buffers) {
                foreach(BufferedTrees bufferedTrees in bufferedTreesCollection.BufferedTrees) {
                    if(impTreesXmlTreeTypes.TryGetValue(bufferedTrees.ID, out treeInfo)){
                        //treePos = new List<Vector3>();

                        foreach(Position pixelPosition in bufferedTrees.Positions) {
                            trees_found++;

                            if (treesCreated > treeLimit) {
                                Debug.Log("Tree limit" + treeLimit + "reached.");
                                break;
                            }
                            ...
                            if (trees_found % step == 0) {
                                if (step < 0)
                                    ...
                                    if (AddTreeFromPosition(pixelPosition.X, pixelPosition.Y, scale, treeInfo, isRasterMode))
                                        treesCreated++;
                                }
                            }
                        ...
                        /*if(treePos.Count > 0) {
                            string msg = "TreesBatch[TreeTypes:";
                            foreach(TreeInfo t in treeInfo) {
                                msg += t.name + ",";
                            }
                            msg += "]:\n";
                            for (int t = 0; t < treePos.Count; t++)
                                msg += t.ToString() + ": x " + treePos[t].x.ToString() + " ,z " + treePos[t].z.ToString() + "\n";
                            Debug.Log(msg);
                            //SimulationManager.instance.AddAction(AddTreesBuffered(treePos.ToArray(), treeInfo.ToArray(), SimulationManager.instance.m_randomizer));
                        }*/
                    }
                }
            }
            ...
            Debug.Log(debug_msg);
            Debug.Log("Trees found: " + trees_found.ToString() + " Trees Created: " + treesCreated.ToString());

            panel.SetMessage("GeoSkylines", "Trees (from XML data) import completed. ", false);


        }

        public bool AddTreeFromPosition(double x, double y, float scale, TreeInfo[] treeInfos, bool isRaster) {
            double gameX = x;
            double gameZ = y;

            if (isRaster) {
                gameX = (x * scale - 8640) + (0.5 * scale) + (0.5 - sysRand.NextDouble()) + impTreesRasterOffsetX;
                gameZ = (y * scale - 8640) + (0.5 * scale) + (0.5 - sysRand.NextDouble()) + impTreesRasterOffsetY;
            }

            if (Math.Abs(gameX) < impTreesCoordMax && Math.Abs(gameZ) < impTreesCoordMax) {
                var treePos = new Vector3((float)gameX, 0f, (float)gameZ);

                ti = treeInfos[sysRand.Next(0, treeInfos.Length)];
        ...
        public void ImportTreesRaster() {
            ...
            string rasterFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impTreesRasterFileName.Trim()));
            ...
            int treeLimit = 250000;
            if (impTreesRasterTreesLimit != 0) {
                treeLimit = impTreesRasterTreesLimit;
            }
            ...
            Debug.Log("height: " + height.ToString() + " width: " + width.ToString());
            if (height != width) {
                string msg = "The size of the provided raster is not equal " + height + " =/= " + width;
                panel.SetMessage("GeoSkylines", msg, false);
                return;
            }
                    ...
                    if (pixel_color.r < 1 || pixel_color.g < 1 || pixel_color.b < 1) {
                        ...
                        if (trees_found % step == 0) {
                            if (step < 0)
                                ...
                                if (AddTreeFromPixels(x, y, scale, treeTypes))
                                    treesCreated++;
                            }
                        }
        ...
        public bool AddTreeFromPixels(int x, int y, float scale, TreeInfo[] treeInfos) {
            double gameX = (x * scale - 8640) + (0.5 * scale) + (0.5 - sysRand.NextDouble()) + impTreesRasterOffsetX;
            double gameZ = (y * scale - 8640) + (0.5 * scale) + (0.5 - sysRand.NextDouble()) + impTreesRasterOffsetY;
        ...
        public void ImportTreesVector() {
            ...
            string impTreesVectorFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impTreesVectorFileName.Trim()));

            if (!File.Exists(impTreesVectorFilePath)) {
                panel.SetMessage("GeoSkylines", impTreesVectorFileName + " file doesn't exist!", false);
                return;
            }

            StreamReader sr = File.OpenText(impTreesVectorFilePath);
        ...
        public void ImportWaterWay() {
            ...
            string impWaterWayFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impWaterWayFileName.Trim()));

            if (!File.Exists(impWaterWayFilePath)) {
            ...
            StreamReader sr = File.OpenText(impWaterWayFilePath);
        ...
        public void ImportWaterReservoirs() {
            ...
            string impWaterFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impWaterFileName.Trim()));

            if (!File.Exists(impWaterFilePath)) {
            ...
            StreamReader sr = File.OpenText(impWaterFilePath);
            ...
            panel.SetMessage("GeoSkylines", "Digging water reservoirs complete. ", false);
        }
        ...
        public void ImportServices() {
            ...
            string impServicesFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impServicesFileName.Trim()));

            if (!File.Exists(impServicesFilePath)) {
            ...
            string serviceMatchFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", serviceMatchFileName.Trim()));
            Dictionary<string, uint> serviceMapping = new Dictionary<string, uint>();
            if (!File.Exists(serviceMatchFilePath)) {
                panel.SetMessage("GeoSkylines", serviceMatchFileName + " file doesn't exist!", false);
                return;
            } else {
                using (StreamReader s_map_sr = File.OpenText(serviceMatchFilePath)) { 
            ...
            StreamReader sr = File.OpenText(impServicesFilePath);
        ...
        public void ImportZones() {
            ...
            string zoneMappingFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", zoneMappingFileName.Trim()));
            Dictionary<string, string> zoneMapping = new Dictionary<string, string>();
            if (!File.Exists(zoneMappingFilePath)) {
                panel.SetMessage("GeoSkylines", zoneMappingFileName + " file doesn't exist!", false);
                return;
            } else {
                StreamReader z_map_sr = File.OpenText(zoneMappingFilePath);
        ...
        public void ImportZonesArea() {
            ...
            string impZonesFilePath = Path.GetFullPath(string.Format("{0}{1}", "Files/", impZonesFileName.Trim()));

            List<GeoSkylinesZone> zones = new List<GeoSkylinesZone>();

            if (!File.Exists(impZonesFilePath)) {
            ...
            StreamReader sr = File.OpenText(impZonesFilePath);
        ...
        public string OutputConfiguration(string type) {
            ...
            if (type == "tree1") {
                ...
                confTxt += "ImportTreesRasterTreesLimit: " + impTreesRasterTreesLimit + "\n";
            }
            ...
            if(type == "tree3") {
                confTxt += "ImportTreesCoordMax: " + impTreesCoordMax + "\n";
                confTxt += "ImportTreesXmlTreeTypes: ";
                foreach (var treeID in impTreesXmlTreeTypes.Keys) {
                    confTxt += treeID + "[";
                    TreeInfo[] treeInfos = impTreesXmlTreeTypes[treeID];
                    foreach(TreeInfo treeT in treeInfos) {
                        confTxt += treeT + ", ";
                    }
                }
                confTxt += "]\n";
                confTxt += "ImportTreesXmlFileName: " + impTreesXmlFileName + "\n";
                confTxt += "ImportTreesXmlOffsetX: " + impTreesXmlOffsetX + "\n";
                confTxt += "ImportTreesXmlOffsetY: " + impTreesXmlOffsetY + "\n";
                confTxt += "ImportTreesXmlMultiply: " + impTreesXmlMultiply + "\n";
                confTxt += "ImportTreesXmlTreesLimit: " + impTreesXmlTreesLimit + "\n";
            }
